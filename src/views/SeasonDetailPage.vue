<script setup>
  ...
  async function load() {
    ...
    try {
      ...
      const [computedSeason, nextSeasons] = await Promise.all([
        getSeasonComputed(props.mode, ver.value, props.id, {
          signal: ac.signal,
        }),
        getSeasons(props.mode, ver.value, {
          starFilter: "all",
          signal: ac.signal,
        }),
      ]);
  ...
  const stageEffectTitle = computed(() =>
    props.mode === "peak" ? "当前关卡效果" : "当前阶段补充效果",
  );
  ...
  const activeStage = computed(() =>
    showStageSelector.value
      ? (data.value?.stages || []).find((s) => s.key === stageKey.value) || null
      : (data.value?.stages || [])[0] || null,
  );

  const isPeakBossStage = computed(
  ...
  const hasTopEffects = computed(() => {
    if (
      data.value?.effects?.length &&
      (props.mode !== "peak" || isPeakBossStage.value)
    )
      return true;
    ...
    return Boolean(
      nodeEffects.side1?.length ||
      nodeEffects.side2?.length ||
      nodeEffects.side3?.length,
    );
  });
      ...
      <div class="hero-main">
        <div class="hero-copy">
          ...
          <p class="hero-desc">
            {{
              mode === "peak"
                ? "按关卡拆分查看每一路敌人、关卡效果与总 HP 构成。"
                : "按节点与波次展开查看怪物图片、弱点、数量与总 HP。"
            }}
          </p>
        </div>
        ...
        <div
          class="season-switch"
          @focusout="scheduleCloseSeasonSwitch"
          @focusin="cancelCloseSeasonSwitch"
          @mouseleave="closeSeasonSwitch">
          ...
          <div class="switch-control">
            ...
            <div
              ...
              role="listbox"
              aria-label="选择赛季"
              @mouseenter="cancelCloseSeasonSwitch">
              <button
      ...
      <section class="detail-top">
        <div class="top-panels">
          ...
          <div
            ...
            <EffectList
              v-if="
                data.effects?.length && (mode !== 'peak' || isPeakBossStage)
              "
              :title="effectTitle"
              :items="data.effects" />
            <template v-if="mode === 'doom'">
              <EffectList
                v-if="data.nodeEffects?.side1?.length"
                title="节点 1 效果"
                :items="data.nodeEffects.side1" />
              <EffectList
                v-if="data.nodeEffects?.side2?.length"
                title="节点 2 效果"
                :items="data.nodeEffects.side2" />
              <EffectList
                v-if="data.nodeEffects?.side3?.length"
                title="星启模式效果"
                :items="data.nodeEffects.side3" />
            </template>
            <EffectList
              v-if="activeStage?.effects?.length"
              :title="stageEffectTitle"
              :items="activeStage.effects" />
          </div>
        </div>
      </section>
          ...
          <div class="block-hd">
            ...
            <div class="block-sub">
              {{
                mode === "peak"
                  ? "选择左侧关卡后，各波次在右侧横向展开"
                  : "按节点与波次展开，已带上怪物图片、弱点与总HP"
              }}
            </div>
          </div>
          <MonsterList
            :stage="activeStage"
            :layout="mode === 'peak' ? 'wave-row' : 'default'" />
        </div>
      </section>
    </div>
  </div>
...
<style scoped>
  ...
  .hero {
    ...
    background:
      linear-gradient(
        135deg,
        color-mix(in oklab, var(--surface-strong) 92%, transparent),
  ...
  .hero::after {
    ...
    background: radial-gradient(
      circle,
      color-mix(in oklab, var(--acc) 24%, transparent) 0%,
      transparent 68%
    );
    pointer-events: none;
  }
  ...
  .hero-total {
    ...
    background: linear-gradient(
      180deg,
      color-mix(in oklab, var(--warn) 10%, var(--surface-soft)),
  ...
  .option-mark {
    ...
    background:
      radial-gradient(
        circle at 50% 50%,
        color-mix(in oklab, var(--warn) 82%, white) 0 28%,
        transparent 29%
      ),
  ...
  .season-option[data-active="true"] .option-mark {
    background:
      radial-gradient(
        circle at 50% 50%,
        color-mix(in oklab, var(--warn) 90%, white) 0 30%,
        transparent 31%
      ),
      linear-gradient(
        180deg,
  ...
  .btn {
    ...
    background: linear-gradient(
      180deg,
      color-mix(in oklab, var(--acc) 18%, var(--bg-1)),
      color-mix(in oklab, var(--acc2) 10%, var(--bg-1))
    );
  ...
  .panel {
    ...
    background: linear-gradient(
      ...
      color-mix(in oklab, var(--surface-soft) 90%, transparent)
    );
  ...
  @media (min-width: 880px) {
    .hero-main {
      grid-template-columns: minmax(0, 1fr) minmax(180px, 240px) minmax(
          260px,
          340px
        );
      align-items: end;
