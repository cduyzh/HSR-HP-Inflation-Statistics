...
- 现有覆盖：预计算趋势命中、单期预计算命中、回退复算时的受控并发（≤6）与输入顺序保持、复算过程中的 `onItems` 分批回填（`tests/endgame-precomputed.test.js`）；本地派生趋势缓存的完整命中（零请求）、部分命中只复算缺口、版本变化后失效并清理旧键（`tests/trend-cache.test.js`）。
...
- 构建注意：echarts 在 `EChartView.vue` 里**按需注册**（`echarts/core` + `LineChart` + `GridComponent` + `TooltipComponent` + `CanvasRenderer`），并由 `HpTrendsPage.vue` 用 `defineAsyncComponent`（带 `loadingComponent` 占位）异步加载，所以它是懒加载分片、不进首屏。实测：echarts 分片 493.5KB（gzip 167KB），首屏 `index` 153.8KB（gzip 57.7KB）；`chunkSizeWarningLimit` 已收紧到 600。
